...
The file helpers close their owned streams on success and failure, including unchanged-content writes. Write-failure cleanup closes the output before deleting the failed file or restoring its backup. Close I/O failures remain logged rather than replacing the primary failure.
