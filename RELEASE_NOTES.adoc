...
== Unreleased updates since `compiler-2026.2`
...
`[N]`
|Post-release Maven state reset after `compiler-2026.2`, plus fixes for broken `AGENTS.md` paths to project documentation.
|Keeps the development line ready for the next snapshot cycle and repairs local documentation links. No user-facing runtime behaviour change.

|2026-04-18
|`origin/sausage/docs-*` +
`[N]`
|Documentation-only branch set covering broken URL replacement, HTTP-to-HTTPS upgrades, copyright-year clean-up, UK English spelling, package-info grammar, AGENTS grammar, and replacement of characters above U+00FF with ASCII equivalents.
|Improves documentation accuracy and repository character-set compliance. No runtime or API impact.

|2026-03-26
|`origin/feature/junit5` +
`[N]`
|Migrates runtime compiler tests to JUnit 5, updates the test dependency setup, and fixes remaining Jupiter migration regressions.
|Modernises the test suite with no identified library runtime behaviour change; the only main-source diff is whitespace cleanup.
|===
